...
为避免源素材手臂末端出现生硬直线，视频会根据首帧 alpha bbox 生成固定的下半圆遮罩：上方 78% 不处理，下方使用半径约为主体高度 23% 的圆弧裁切，并加入横向 10 px、纵向 24 px 羽化。遮罩在整个片段中保持固定，避免逐帧追踪造成抖动；最终 MP4 不含 Alpha，被裁掉的区域直接显示 BG71。
